<template>
  <div class="marketplaceSection section-modern" v-if="models.length">
    <div class="marketplace-container">
      <div class="section-header-modern">
        <h2>Trending NFTs</h2>
        <p>Discover the most popular digital assets in our marketplace</p>
      </div>

      <div class="nowrap-items" v-dragscroll.x>
        <ul class="products modern-grid">
          <li v-for="(model,index) in models" :key="index">
            <div class="product-item modern-card">
              <nuxt-link :to="`/products/${model.id}`">
                <div class="product-image" :style="`background-image: url(${model.image_full});`">
                  <div class="product-overlay">
                    <span class="badge-modern badge-primary">View Details</span>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    getModels(){
      this.listItems('trending').then(res => {
