<template>
  <div class="inner-container" v-loading="loadRunning">
    <div class="whitepaper-container">
      <div class="whitepaper-sidebar">
        <div class="whitepaper-sidebar-inner">
          <div class="whitepaper-sidebar-head">
            <div class="w-sidebar-title">Whitepaper</div>
      ...
      <div class="whitepaper-content" v-if="models && models.length">
        <div class="whitepaper-content-min mb-5" v-for="(item,index) in models" :key="index" :ref="`content-${item.id}`">
          <div class="whitepaper-content-head">
            <h1 class="whitepaper-content-title">{{item.title}}</h1>
          </div>
          <div class="whitepaper-content-txt" v-html="item.content"></div>
...
export default {
  ...
  watch:{
    modelId(val){
      if(this.$refs['content-'+val] && this.$refs['content-'+val].length){
        this.$refs['content-'+val][0].scrollIntoView({ behavior: "smooth" });
      }
  ...
  updated(){
    if(this.modelId && !this.firstUpdated){
      if(this.$refs['content-'+this.modelId] && this.$refs['content-'+this.modelId].length){
        setTimeout(() => {
          this.$refs['content-'+this.modelId][0].scrollIntoView({ behavior: "smooth" });
        },300)
  ...
  methods: {
    ...
    getModels(){
      this.list().then(res => {
        this.models = orderBy(res['data'].map(item => {
          item['order'] = Number((item['parent_id']?item['parent_id']:'')+item['id']);
          return item;
        }), 'order');
        this.getNavItems();
        this.openedItems = this.getActiveItems(this.modelId);
